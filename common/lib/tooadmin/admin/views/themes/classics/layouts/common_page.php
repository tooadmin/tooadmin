	...
	<body>
		<?php $this->beginContent(TDCommon::getRender('layouts/common_topbar')); $this->endContent(); ?>
 		<div class="container-fluid">
        		...
        		<div class="row-fluid">
				<?php $mainSpanNum=10; $topMnInd = isset($_GET["topmnInd"]) ? intval($_GET["topmnInd"]) : -1; 
				...
				if(count($menuChilds) > 0) { ?>
				<div style="<?php echo TDSessionData::spec_layout_common_page_left(); ?>" class="span2 main-menu-span">
				<div class="well nav-collapse sidebar-nav">
					<ul class="nav nav-tabs nav-stacked main-menu">
					<?php foreach($menuChilds as $secondIndex => $second) {
						...
						if(TDSessionData::getCurIsDevModel()){
							$devMD = '<div class="btn-group pull-right" style="position: absolute;margin-left: 76px;margin-top: -20px;">
							...
							<ul class="dropdown-menu qkmenu">
								<li onclick="quickEditMenu('.$second['id'].')"><span class="icon icon-color icon-edit"></span>'.TDLanguage::$quikEditMenu_edit.'</li>
								<li onclick="quickAddMenu('.$second['id'].')"><span class="icon icon-color icon-plus"></span>'.TDLanguage::$quikEditMenu_addChild.'</li>
				'.($secondIndex != 0 ? '<li onclick="quickReorderMenu('.$second['id'].',0)"><span class="icon icon-orange icon-arrowthick-n"></span>'.TDLanguage::$quikEditMenu_toUp.'</li>' : '').'
				'.($secondIndex < count($menuChilds)-1 ? '<li onclick="quickReorderMenu('.$second['id'].',1)"><span class="icon icon-orange icon-arrowthick-s"></span>'.TDLanguage::$quikEditMenu_toDown.'</li>' : '').'
				<li onclick="quickDeleteMenu('.$second['id'].')"><span class="icon icon-color icon-close" ></span>'.TDLanguage::$quikEditMenu_delete.'</li>
							</ul>
							</div>';
						}	
						?>
						<li class="nav-header hidden-tablet" style="font-size:12px;"><?php echo $second['name'].$devMD; ?></li>
						<?php 	foreach($threeChilds as $threeIndex => $row) { echo TDPathUrl::getMenuItemLink($row['id'],$topMnInd,0,TDPathUrl::$menuForType_commonPage,$threeIndex, count($threeChilds)); } ?>
					<?php } ?>
					</ul>
				...
				<div style="<?php echo TDSessionData::spec_layout_common_page_right(); ?>" id="content" class="span<?php echo $mainSpanNum; ?>">
