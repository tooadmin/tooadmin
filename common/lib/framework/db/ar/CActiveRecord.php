...
abstract class CActiveRecord extends CModel
{
	...
	public function __get($name)
	{
		if(isset($this->_attributes[$name]))
			return $this->checkFormulaAttribute($name); //$this->_attributes[$name];
		...
		elseif (strrpos($name,TDStaticDefined::$childModels) !== false) {
			$tbName = str_replace(TDStaticDefined::$childModels,"",$name);
			$tbName = explode(TDStaticDefined::$foreignKey_tableName,$tbName);
			$tbfk = $tbName[0];
			$tbName = isset($tbName[1]) ? $tbName[1] : "";
			if(!empty($tbfk) && !empty($tbName) && TDSessionData::checkIsTableName($tbName)) {
				$prk = $this->getPrimaryKey();
				$this->_attributes[$name] = TDModelDAO::getModel($tbName)->findAll("`".$tbfk."`=".(empty($prk) ? 0 : $prk));
				return $this->_attributes[$name];
			}	
		} elseif(strrpos($name,TDStaticDefined::$foreignKey_tableName) !== false) {
			$tbName = explode(TDStaticDefined::$foreignKey_tableName,$name);
			$tbfk = $tbName[0];
			$tbName = isset($tbName[1]) ? $tbName[1] : "";
			if(!empty($tbfk) && !empty($tbName) && TDSessionData::checkIsTableName($tbName)) {
				$prk = $this->$tbfk;
				$this->_attributes[$name] = TDModelDAO::getModel($tbName,$prk,true);
				return $this->_attributes[$name];
			}		
		} elseif($name == "getPrimaryKey") {
	...
	/*
	 * (custom column) formula compute 
	 */
	public function checkFormulaAttribute($name) {
		if(empty($this->_attributes[$name]) && $this->_attributes[$name] !== 0) {
			if(isset($this->getMetaData()->columns[$name])) {
				$column = $this->getMetaData()->columns[$name];
				if($column->comment == TDTableColumn::$COLUMN_TYPE_COMMENT_CUSTOM_STR) {
					$this->_attributes[$name] = Fie_formula::computeFormula($this,$name);	
				}
			} 
		}	
		return $this->_attributes[$name];
	...
	{
		/*使用缓存 ，因为 表单存在时验证规则 必填项是临时生成的所以不能使用缓存
		if(isset(self::$_models[$className])) {
			return self::$_models[$className];
		} else {
			if(!empty($tableName)) { 
				$str = "md_".$tableName; 
				if(!isset(self::$_models[$str])) {
					$model = new $str(); 
					$model->_md=new CActiveRecordMetaData($model);
					$model->attachBehaviors($model->behaviors());
					self::$_models[$str] = $model;
				}
				return self::$_models[$str];
			} else {
				$model= new $className(empty($tableName)?null:$tableName);
				$model->_md=new CActiveRecordMetaData($model);
				$model->attachBehaviors($model->behaviors());
				return $model;
			}
