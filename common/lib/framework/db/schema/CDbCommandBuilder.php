...
class CDbCommandBuilder extends CComponent
{
	...
	public function createFindCommand($table,$criteria,$alias='t')
	{
		...
		$sql=$this->applyOrder($sql,$criteria->order,$table);
		$sql=$this->applyLimit($sql,$criteria->limit,$criteria->offset);
		TDSqlMonitor::commonSQL($sql);
	...
	public function createInsertCommand($table,$data)
	{
		...
		foreach($data as $name=>$value)
		{
			...
			{
				if($column->comment == TDTableColumn::$COLUMN_TYPE_COMMENT_CUSTOM_STR) {
					continue;
				}
	 ...
	 */
	public function applyOrder($sql,$orderBy,$table = null)
	{
		if($orderBy!='') {
			//return $sql.' ORDER BY '.$orderBy;
			if(strpos($sql,"`t`") != false && strpos($sql,"`t`.*") === false && strpos($sql,"t.*") === false) {
				$sql = str_replace("*"," `t`.* ",$sql);
			}
			return $sql.$this->orderSQL($orderBy,$table);
		} else {
			return $sql;
		}
	}
	public function filterSpecialKey($str) {
		$str = str_replace('`','',$str);
	...
	public function orderSQL($orderBy,$table = null) {
		$result =  '';
		if(empty($orderBy)) {
			return '';
		}
		...
		$baseOrder = '';
		$joinOrder = '';
		$array0 = explode(',',$orderBy);
		for($i=0; $i<count($array0); $i++) {
			$array = explode('->',$array0[$i]);
			//check is custom column start
			$checkTable = $table;
			if(count($array) > 1) {
				$tmpcheckstr = $array[count($array)-2];
				$tmpcheckstr = explode(TDStaticDefined::$foreignKey_tableName,$tmpcheckstr);
				$tmpcheckstr = $tmpcheckstr[1];
				$checkTable = TDTable::getTableObj($tmpcheckstr);
			}
			$checkColumnName = $this->filterSpecialKey($array[count($array)-1]);
			if(isset($checkTable->columns[$checkColumnName])) {
				$tmpcol = $checkTable->columns[$checkColumnName];
				if($tmpcol->comment == TDTableColumn::$COLUMN_TYPE_COMMENT_CUSTOM_STR) {
					$formatResult = Fie_formula::formatFormulaToOrderAndLeftJoinSQL($table->name,$this->filterSpecialKey($array0[$i]));
					if(!empty($formatResult['formula'])) {
						$result = $formatResult['leftJoin'].' order by '.'('.$formatResult['formula'].')';
						if(strripos(yii::app()->request->url,'.desc') === false) { 
							$result .= ' desc';		
						}
					} else {
						$result = '';
			...
			if(count($array) == 1) {
				if(!empty($baseOrder)) 
					$baseOrder .= ',';
				$OrderStr = str_replace('`t`.','',$array[0]);
				$OrderStr = str_replace('`','',$OrderStr);
				$OrderStr = trim($OrderStr);
				$tmpa = explode(' ',$OrderStr);
				$baseOrder .= '`t`.`'.$tmpa[0].'`';
				if(isset($tmpa[1])) {
					$baseOrder .= ' '.$tmpa[1];	
				}
				...
				for($c=0; $c < count($array); $c++) {
					if($c != count($array) - 1) {
						...
						if($c == 0) {
							...
							$tmpTB = explode(TDStaticDefined::$foreignKey_tableName,$array[$c-1]);
							$tmpTB = isset($tmpTB[1]) ? $tmpTB[1] : $tmpTB[0]; 
							$tables = array($connection->getSchema()->getTable($tmpTB,$connection->schemaCachingDuration!==0));
						...
						if(!empty($sptable)) {
							foreach($sptable->foreignKeys as $forkey => $keyData) {
								if($forkey.TDStaticDefined::$foreignKey_tableName.$keyData[0] == $array[$c]) {	
									$result .= ' left join `'.$keyData[0].'` as `'.$array[$c]
									.'` on  `'.$array[$c].'`.`'.$keyData[1].'`='.$tbAkStr.'.`'.$forkey.'` ';
								}
							}
						}
					} else {
						$array[$c] = str_replace('.desc','',$array[$c]);
						$tmpa = explode(' ',  trim($array[$c]));
						$joinOrder = '`'.$array[$c-1].'`.`'.$tmpa[0].'`';	
						if(isset($tmpa[1])) {
							$joinOrder .= ' '.$tmpa[1];	
						}
					}
		...
		if(!empty($joinOrder)) {
			...
			$baseOrder = $joinOrder.$baseOrder;	
		}
		if(!empty($baseOrder)) {
			$result .= ' ORDER BY '.$baseOrder;
		}
		return $result;
	}
	...
	protected function ensureTable(&$table)
	{
		...
		if(is_string($table) && ($table=  TDTable::getTableObj($tableName=$table))===null)
