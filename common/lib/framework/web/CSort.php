...
class CSort extends CComponent
{
	...
	public function getForeignTableOrderBy() {
		$attributes = isset($_GET[$this->sortVar]) ? $_GET[$this->sortVar] : '';
		if(strrpos($attributes,'->') !== false) {
	...
	public function currentForginIsAscOrder() {
		$attributes = isset($_GET[$this->sortVar]) ? $_GET[$this->sortVar] : '';
		if(strrpos($attributes,'->') !== false) {
			if(strripos(yii::app()->request->url,'.desc') === false) { 
				return true;
	...
	{
		$foreignkeyOrder = $this->getForeignTableOrderBy();
		if(!empty($foreignkeyOrder)) {
			return $foreignkeyOrder;
		}
		...
		else
		{
			if($this->modelClass!==null)
				$schema=CActiveRecord::model($this->modelClass,$this->tableName)->getDbConnection()->getSchema();
			$orders=array();
				...
				elseif($definition!==false)
				{
					...
					if(isset($schema))
					{
						if(($pos=strpos($attribute,'.'))!==false)
							...
							$attribute=($criteria===null || $criteria->alias===null ? CActiveRecord::model($this->modelClass,$this->tableName)->getTableAlias(true) : $schema->quoteTableName($criteria->alias)).'.'.$schema->quoteColumnName($attribute);
					}
	 ...
	 */
	public function link($attribute,$label=null,$htmlOptions=array(),$tableName=null)
	{
		...
		if(isset($_GET['is_new_column_order']) && $_GET['is_new_column_order'] == 1) {
			if(strripos(yii::app()->request->url,'.desc') !== false) { 
				$url = str_replace('.desc','',$url);				
			} else  {
				$url = $url.'.desc';			
			}
		}
	...
	public function resolveLabel($attribute)
	{
		...
		if($this->modelClass!==null)
			...
			return CActiveRecord::model($this->modelClass,$this->tableName)->getAttributeLabel($attribute);
		else
			return $attribute;
	}
	...
	public function createUrl($controller,$directions)
	{
		$sorts=array();
		foreach($directions as $attribute=>$descending)
			$sorts[]= ($this->currentForginIsAscOrder() || $descending) ? $attribute.$this->separators[1].$this->descTag : $attribute;
	...
	public function resolveAttribute($attribute)
	{
		...
		elseif($this->modelClass!==null)
			...
			$attributes=CActiveRecord::model($this->modelClass,$this->tableName)->attributeNames();
		else
			return false;
		foreach($attributes as $name=>$definition)
		{
			...
			elseif($definition==='*')
			{
				...
				if($this->modelClass!==null && CActiveRecord::model($this->modelClass,$this->tableName)->hasAttribute($attribute))
					return $attribute;
			}
