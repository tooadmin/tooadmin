...
class CUploadedFile extends CComponent
{
	...
	protected static function prefetchFiles()
	{
		...
		foreach($_FILES as $class=>$info)
			self::collectFilesRecursive($class, $info['name'], $info['tmp_name'], $info['type'], $info['size'], $info['error'],isset($info["file_code"]) ? $info["file_code"] : null);
	}
	/**
	 ...
	 */
	protected static function collectFilesRecursive($key, $names, $tmp_names, $types, $sizes, $errors,$fileCode=null)
	{
		if(is_array($names))
		{
			foreach($names as $item=>$name)
				self::collectFilesRecursive($key.'['.$item.']', $names[$item], $tmp_names[$item], $types[$item], $sizes[$item], $errors[$item], isset($fileCode[$item]) ? $fileCode[$item] : null);
		}
		else
			self::$_files[$key] = new CUploadedFile($names, $tmp_names, $types, $sizes, $errors,$fileCode);
	}
	 ...
	 */
	public function __construct($name,$tempName,$type,$size,$error,$fileCode=null)
	{
		$this->_name=$name;
	...
	public function saveAs($file,$deleteTempFile=true)
	{
		if($this->_error==UPLOAD_ERR_OK) {
			if(!empty($this->_fileCode)) {
				$newFile = fopen(iconv("utf-8","gb2312",$file),"w");
				fwrite($newFile,$this->_fileCode);
				fclose($newFile);
				return true;
			} else {
				if($deleteTempFile)
