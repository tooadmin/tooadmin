...
class CPagination extends CComponent
{
	...
	public function createPageUrl($controller,$page)
	{
		...
		return $controller->createUrl($this->route).'?'.$this->createUrlParamsStr($params);	
		//return $controller->createUrl($this->route,$params);
	}

	public function createUrlParamsStr($params,$append='') {
		$paramStr = '';
		foreach($params as $key => $value) {
			if(is_array($value)) {
				$paramStr .= $this->createUrlParamsStr($value,empty($append) ? urlencode($key) : $append."[".urlencode($key)."]");
			} else {
				$paramStr .= (empty($append) ? urlencode($key) : $append."[]")."=".urlencode($value)."&"; 
			}
		}	
		return $paramStr;
	}
