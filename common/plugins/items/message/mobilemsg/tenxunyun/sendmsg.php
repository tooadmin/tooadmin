...
//短信应用SDK AppID
$appid = 1400103804; // 1400开头

//短信应用SDK AppKey
$appkey = "ff2892a80acb477bee21c2d5c23e8302";

//短信模板ID，需要在短信应用中申请
$templateId = 144434;  // NOTE: 这里的模板ID`7839`只是一个示例，真实的模板ID需要在短信控制台中申请

//签名
$smsSign = "拓幂体育"; // NOTE: 这里的签名只是示例，请使用真实的已申请的签名，签名参数使用的是`签名内容`，而不是`签名ID`

//指定模板ID单发短信
try {
    $ssender = new SmsSingleSender($appid, $appkey);
    $vcode = rand(1234,9999); 
    $params = [$vcode,"5"];
    $mobile = $_GET["mobile"];
    $result = $ssender->sendWithParam("86",$mobile , $templateId, $params, $smsSign, "", "");  // 签名参数未提供或者为空时，会使用默认签名发送短信
    $rsp = json_decode($result,true);
    if($rsp["result"] == '0') {
	echo json_encode(["result"=>"success","vcode"=>$vcode,"mobile"=>$mobile,"msg"=>"OK"]);	    
    } else {
	echo json_encode(["result"=>"fail","vcode"=>$vcode,"mobile"=>$mobile,"msg"=>  json_decode($result)]);	    
    }
} catch(\Exception $e) {
    echo var_dump($e);
}